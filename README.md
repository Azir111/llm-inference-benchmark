...
> 其余完全不变——QPS 就从近线性扩展（1.22 → 10.21）退化为与朴素 Transformers
> 一样全程卡死（~1.3）。
...
|------|------|------|------|
| 首 Token 延迟 TTFT_P90 | **54.9 ms** | 13757 ms | 29207 ms |
| 吞吐量 QPS | **10.21** | 1.30 | 0.59 |
| 单 Token 间隔 TPOT | 13.3 ms | 12.5 ms | 31.2 ms |
| 吞吐扩展性（并发 1→16） | 1.22 → 10.21（近线性） | ~1.3（无扩展） | ~0.5（无扩展） |

**结论（每条都对应一个被隔离出来的变量）**

1. **扩展性来自 continuous batching（A vs B）**：仅去掉 batching 这一个变量，并发=16 时 QPS 从 10.21 跌到 1.30、TTFT_P90 从 54.9ms 飙到 13.8s。B 与朴素 Transformers（C）一样毫无扩展性——证明吞吐扩展的根本来源是调度机制，不是「换了个框架」。
2. **单 token 速度由 kernel + 精度决定，与 batching 无关（A vs B 的 TPOT）**：A 与 B 的 TPOT 几乎相同（13.3 vs 12.5ms），说明 batching 不影响吐字快慢；而 B（FP16）比 C（4bit）的 TPOT 快约 2.5×，这部分才是 kernel 优化 + 反量化开销的贡献。
3. **端到端 681 级别的总差距（A vs C）仅作总览**：它混合了 batching、kernel、量化三重因素，不可单独归因，故不再作为主打数字。
...
并发=1 时只有一个请求，batching 开不开都没区别。实测 A / B 的 TTFT_P90 为 28.3 / 28.1 ms，QPS 为 1.22 / 1.25——两者重合，验证消融实验没有引入额外变量，后续结论才站得住。
...
只有 A（开 batching）随并发近线性爬升；B 和 C 都贴地平走。**A 与 B 唯一的差别就是 `max_num_seqs`**，所以这条 8× 的扩展性差距可以 100% 归因于 continuous batching。
...
|------|------|------|------|
| 1    | 28.3 ms  | 28.1 ms   | 58.5 ms |
| 2    | 39.0 ms  | 1072 ms   | 3340 ms |
| 4    | 39.2 ms  | 3730 ms   | 8761 ms |
| 8    | 40.0 ms  | 7066 ms   | 17815 ms |
| 16   | **54.9 ms** | **13757 ms** | **29207 ms** |

A 全程平稳（28→55ms）。B 一旦关掉 batching，TTFT 立刻随并发飙升——请求开始排队，排队时间全计入首 token 延迟，这正是 continuous batching 消除的部分。
...
| 组 | TPOT_mean（并发 1→16） |
|------|------|
| A: vLLM (batching, FP16) | 12.9 → 13.3 ms |
| B: vLLM (单流, FP16)     | 12.5 → 12.5 ms |
| C: Transformers (4bit)   | 30.9 → 31.2 ms |

三组各自都不随并发变化。关键看点：**A 与 B 的 TPOT 几乎相同**（都是 vLLM FP16），证明 **batching 不影响单 token 速度**；而 B（FP16）比 C（4bit）快约 2.5×，这才是 kernel + 量化的贡献。
...
C 组的 ITL_P99 高达 113~145ms，远高于其 TPOT 均值（31ms），说明 4bit 串行推理存在明显的**偶发吐字卡顿**；
A / B 的 ITL_P99 仅 13~15ms，与各自 TPOT 贴合，吐字均匀。vLLM 不只是更快，流式输出也更稳。

### 核心结论

**1. 扩展性来自 continuous batching（A vs B）**：仅改 `max_num_seqs` 一个参数，并发=16 时 QPS 相差 8×、TTFT_P90 相差 251×。串行架构下第 N 个请求必须排队等前面所有请求整段生成完；Continuous Batching 让新请求在下个 decode step 即可入批，几乎零排队。

**2. 单 token 速度与 batching 无关（A vs B 的 TPOT）**：A、B 的 TPOT 几乎相同（~13ms），batching 不带来单 token 边际成本；B 对 C 的 TPOT 优势（2.5×）来自 vLLM 的 fused kernel + 对照组 4bit 的反量化开销，两者叠加、未进一步隔离。
...
请求2 等待...等待...████████████████ 完成
请求3                 等待...████████████████ 完成
任意时刻只有 ~1 个请求在 GPU 上，吞吐恒等于单请求速度
...
本项目的 A vs B 消融正是这张图的实测证据：B（单流）QPS 恒为 ~1.3，A（batching）爬到 10.21。
...
A: 普通 Static Batching 需要凑齐一批请求才能推理，先到的必须等后到的，GPU 空闲等待；Continuous Batching 在每个 decode step 都能动态加入新请求、释放已完成的请求，GPU 始终满载，高并发下吞吐接近线性扩展。本项目用 `max_num_seqs=1` 关掉它做对照，QPS 直接从 10.21 跌回 1.3，定量验证了这一点。
