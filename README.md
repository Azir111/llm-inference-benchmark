...
> RTX 4060Ti (8GB) ｜ Qwen2.5-1.5B-Instruct（SFT LoRA + DPO 合并）｜ 贪心解码，固定评测集（template 33 + hard 28），裁判 DeepSeek-chat。
...
| 对比 | 桶 | FP16 胜 | 平局 | 4bit 胜 | 解读 |
|------|----|--------|------|--------|------|
| **FP16 vs 4bit** | template | 2 | **29** | 2 | 绝大多数判平，胜负对半 → 无系统性差异 |
| **FP16 vs 4bit** | hard | 6 | **17** | 5 | 同样以平局为主，胜负接近 → 无系统性差异 |

两个桶都是**平局占多数、胜负基本对半开**：裁判在量化前后的两份回答之间看不出谁更好。注意不要单看「胜率」百分比——FP16 胜率低不是因为它差，而是因为 平局占大头，而平局正是这里想要的结论：**质量等价**。

### 逐条多维打分（1~5，均分）

| 模型 | 桶 | 正确性 | 有用性 | 语气 | 安全 | 总均分 |
|------|----|--------|--------|------|------|--------|
| FP16 | template | 3.48 | 3.30 | 4.15 | 4.73 | 3.92 |
| 4bit | template | 3.39 | 3.15 | 4.18 | 4.70 | 3.86 |
| FP16 | hard | 3.25 | 3.04 | 3.96 | 4.25 | 3.62 |
| 4bit | hard | 3.32 | 3.04 | 3.89 | 4.46 | 3.68 |

两组总均分持平（**FP16 3.77 / 4bit 3.77**），分桶最大差距仅 0.06~0.07，在 LLM-as-judge + 几十条样本下属测量噪声，不构成优劣。4bit 在 hard 桶 correctness/safety 微高也属小样本波动，不解读为「量化提升了质量」。

### 延迟 × 质量

![延迟 × 质量](quality/outputs/latency_quality_scatter.png)

| 组 | TPOT（单 token 延迟） | QPS（并发=16） | 质量（总均分） |
|------|------|------|------|
| **FP16**（A: vLLM） | 15.0 ms | **12.91** | 3.77 |
| 4bit（C: Transformers） | 35.8 ms | 0.47 | 3.77 |

图的形状一目了然：**两点在同一高度（质量持平，均 3.77），只在横轴上拉开**。一句话概括——**质量持平，而 FP16 的吞吐（QPS@16）约为 4bit 的 27 倍（12.91 vs 0.47），单 token 延迟也快 2.7×**。

### 核心结论

**1. 4bit 相对 FP16 质量中性（成对胜负 + 逐维打分一致）**：两个桶均以平局为主、胜负对半，总均分 3.77 持平。量化没有带来任何可观测的质量收益。

**2. 因此在这张卡上应部署 FP16，而非 4bit**：FP16 本就塞得进 8GB，4bit 不更好却更慢、吞吐低一个数量级——量化在这里两头不讨好。4bit 只在显存装不下 FP16 时（如换 7B 模型）才值得用，届时它是「拿吞吐/延迟换显存」的手段，而本实验说明这笔交易在**质量侧是安全的**。


### 已知局限

- **评测集小且合成**：template 33 + hard 28，胜负差几条就会抖动，故结论下成「质量无显著差异」，不写成「4bit 一定不掉质量」。
- **量化与框架未分离**：4bit 组为 Transformers-NF4，相对 vLLM-FP16 同时差了精度与框架数值路径两处（与延迟轴 B-vs-C 同一口径，量化为主导因素）。纯量化隔离需对照 vLLM 内 4bit（AWQ/GPTQ），列入后续工作。
- **裁判自身偏差**：LLM-as-judge 与人工标注存在偏差，条件允许时应抽样人工复核校准。
---
